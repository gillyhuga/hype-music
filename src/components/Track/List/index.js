...
const List = ({ index, id, title, artists, album, duration, image, buttonSelect, textSelect }) => {
    ...
    return (
        <tr key={id} className="hover:bg-slate-50/10 hover:rounded-xl ">
            ...
            <td>{textSelect ? <button className="bg-white text-black  border border-white rounded-full py-2 px-8 hover:bg-gray-200 " onClick={setSelect}>Selected</button> :
                <button className="bg-green-500 text-white border  rounded-full py-2 px-8 hover:bg-gray-2000 " onClick={setSelect}>Select</button>}</td>
        </tr>
    )
}
